...
VACASK is run with `-qp` (quiet progress, appropriate for a batch run) and `-sp` (skip postprocessing). The `postprocess(PYTHON, ...)` steps declared in the testbenches are instead executed by the Makefile, which reads them back out of the generated netlist and runs each one with `python3` in the netlist directory. The testbench stays the single source of truth for its postprocessing (as in [snp2le](https://github.com/iic-jku/snp2le)), while VACASK's own subprocess launcher is bypassed: it aborts with a `boost::asio` "Bad file descriptor" error on hosts whose kernel or container runtime blocks the syscalls it uses to spawn and await a child process.
