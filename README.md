...
Run `make build-layout FREQ=<GHz>` before the EM targets. The GDS files, and every Palace output derived from them, are named after the structure and the design frequency only (`sparx_blc_160GHz`, `sparx_wpd_160GHz`, `sparx_bpf_160GHz`, `sparx160_core`), so re-parametrizing a structure in `scripts/six_port_gen.py` does not rename anything. If the file for the requested frequency does not exist, the target aborts with a hint to run `build-layout` first.
...
```sh
make view-em-sim
make view-em-sim FILE_NAME=sparx_blc_160GHz.s4p
```
...
```sh
make copy-sparam SPARAM=sparx_bpf_160GHz
make copy-sparam SPARAM=sparx_wpd_160GHz
make copy-sparam SPARAM=sparx_blc_160GHz
```
...
- `SNP` is the input Touchstone file (default: the de-embedded BPF EM result `verification/em/s-parameter/sparx_bpf_160GHz_deembedded.s2p`).
...
```sh
make snp2le SNP=verification/em/s-parameter/sparx_bpf_160GHz_deembedded.s2p ORDER=13 LE_FORMAT=spice LE_OUT=netlist/spice/sparx_bpf_le.spice
make snp2le SNP=verification/em/s-parameter/sparx_wpd_160GHz_deembedded.s3p ORDER=10 LE_FORMAT=spice LE_OUT=netlist/spice/sparx_wpd_le.spice
make snp2le SNP=verification/em/s-parameter/sparx_blc_160GHz_deembedded.s4p ORDER=6 LE_FORMAT=spice LE_OUT=netlist/spice/sparx_blc_le.spice
