...
Run `make build-layout FREQ=<GHz>` before the EM targets. The parameters below select which GDS in `verification/em/layout/` is simulated, since `six_port_gen.py` encodes them in the file name and `palace_sim.py` derives the frequency, the metal layers, and the reference impedance back from it. If no matching file exists, the target aborts with a hint to run `build-layout` first.
...
Of these, `NP` is a pure solver setting and `FREQ` is the only one that `build-layout` takes as an argument. All others describe how `scripts/six_port_gen.py` builds the structures and are hardcoded there, so on the EM targets they only select the file to simulate. To simulate a variant, change the value in `scripts/six_port_gen.py`, rerun `build-layout`, and call the EM target with the matching parameters.
...
The additional `CONFIG` parameter selects the divider configuration (`C` or `U`, default: `U`), matching `wpd_shape` in `scripts/six_port_gen.py`.

```sh
make build-layout && make sim-wpd-em
make build-layout FREQ=77 && make sim-wpd-em FREQ=77
make sim-wpd-em FREQ=77 CONFIG=C   # after setting wpd_shape = "C" in six_port_gen.py
```

**Hairpin Coupled-Line Bandpass Filter (BPF):**

The additional filter parameters are `BANDWIDTH` (filter bandwidth in GHz, default: `1`), `FILTER_TYPE` (`butter`, `cheby`, or `ellip`, lowercase, default: `butter`), `FILTER_ORDER` (filter order, default: `3`), and `RIPPLE_DB` (passband ripple in dB for Chebyshev and elliptic filters, default: `3`). `RIPPLE_DB` only appears in the GDS file name of the non-Butterworth filters. The 10 µm connection pieces at the filter ports are not exposed as a parameter. They are hardcoded in `scripts/six_port_gen.py`.

```sh
make build-layout && make sim-bpf-em
make build-layout FREQ=77 && make sim-bpf-em FREQ=77
make sim-bpf-em FREQ=77 BANDWIDTH=2 FILTER_TYPE=cheby FILTER_ORDER=5 RIPPLE_DB=1   # after setting the same values in six_port_gen.py
```

**Six-Port Core:**

Simulates the assembled six-port core from `verification/em/layout/sparx<FREQ>_core.gds` (the six-port network with the seven Palace port markers, written by `build-layout`). Since the core file name does not encode the EM parameters, the target passes them to `palace_sim.py` explicitly.

The band pass filter parameters of the core (`order`, `bandwidth`, `filter_type`, `connection_length_bpf`, and `ripple_dB`) are not exposed as parameters. They are hardcoded in `scripts/six_port_gen.py` and must be changed there if required.
