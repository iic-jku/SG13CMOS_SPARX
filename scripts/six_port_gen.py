...
# derive output filenames and cell names from CLI arguments
top_gds_filename = resolve_output_path(args.gds_filename)
top_gds_filename_flat = top_gds_filename.with_name(
    f"{top_gds_filename.stem}_flat{top_gds_filename.suffix}"
)
powdet_gds_filename = resolve_output_path(args.powdet_gds_filename)
powdet_gds_filename_flat = powdet_gds_filename.with_name(
    f"{powdet_gds_filename.stem}_flat{powdet_gds_filename.suffix}"
)
top_cell_name = top_gds_filename.stem
...
PIXEL_SIZE = 2 # pixel size for logo conversion (um)
...
supervisors_png_path = logo_dir / "supervisors.png"
supervisors_gds_path = logo_dir / "supervisors.gds"

tm2_layer = ihp.tech.LAYER.TopMetal2drawing
tm2_foreground = f"{tm2_layer[0]}/{tm2_layer[1]}"
