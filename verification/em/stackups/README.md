...
All eight `.xml` files in `workflow/` are the **same kind of file — technology stackup definitions**.
...
| **PCB** | `pcb_ro4003` | Not an IC at all — a 2-layer Rogers RO4003 board. |
...
| `<Dielectrics>` | The vertical dielectric stack, **ordered top → bottom**, each with a `Thickness` in µm. This builds the simulation box. |
| `<Layers>` | The GDS-layer → physical-layer mapping: `Layer=` is the GDSII layer number, `Type=` is `conductor` / `via` / `dielectric`, plus `Zmin` / `Zmax` and the `Material`. Preceded by `<Substrate Offset="…"/>`, which shifts all metal z-coordinates so that z = 0 in the layer table (wafer surface) lands at the correct absolute height inside the dielectric stack. |

So in one sentence: **materials + z-geometry + GDS layer map.**
Same idea as the openEMS IHP XML files; the `workflow/README.md` notes they are similar but
...
| `pcb_ro4003.xml` | Not an IC at all: 2-layer Rogers RO4003 PCB (εr = 3.38, tanδ = 0.0022), 510 µm core, 17 µm copper top and bottom | — | — |
...
lossy silicon (σ = 2 S/m) is inside the simulation domain. Thicker → more substrate loss captured,
larger mesh, slower simulation.
...
- **Transmission lines, baluns, Butler matrices** — anything with a solid Metal1 ground plane
  underneath → use **`nosub`**. The ground shields the silicon, so removing it costs almost nothing
  in accuracy and saves a lot of mesh. The user's guide explicitly notes the with/without-substrate
  difference was verified negligible for their line models.
- **Inductors, MIM caps**, anything where substrate loss / coupling matters → use **`200um`**
